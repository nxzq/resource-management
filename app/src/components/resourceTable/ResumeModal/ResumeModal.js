...
export default React.memo(function ResumeModal({ FirstName, LastName, id }) {
  ...
  const getResume = () => {
    axios({
      url: `http://localhost:5353/api/resources/${id}/resume`,
      method: 'GET',
      responseType: 'blob',
      headers: new Headers({
      ...
      >
          ...
          {loader ? (
            ...
            >
              ...
              <FormText color="muted">
                Resume has not been uploaded or was uploaded in the wrong
                format.
              </FormText>
            </div>
          ) : (
        ...
        <ModalFooter>
          {loader ? null : data === '' ? (
            <Button
              color="primary"
              className="blue-button disabledBtn shadow-none"
              disabled
            >
              Download
            </Button>
