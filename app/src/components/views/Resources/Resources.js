...
export default function Resources() {
  const { neededSkills, setNeededSkill, showSkillMatch, setShowSkillMatch } =
  ...
  const getSkillMatch = React.useCallback(
    (skills) => {
      let count = 0
  ...
  const getMatchedSkills = React.useCallback(
    (skills) => {
  ...
  const getUnmatchedSkills = React.useCallback(
    (skills) => {
      let personalSkills = skills.map((value) => value)
      let jobSkills = neededSkills.map((value) => value)
      let unmatched = jobSkills.filter(
        (value) => !personalSkills.includes(value)
      )
    ...
    <tr key={person.Id}>
      ...
      {showSkillMatch ? (
        <td>
          <div>
            <SkillCollapse
              matched={getMatchedSkills(person.Skills)}
              unmatched={getUnmatchedSkills(person.Skills)}
              key={'SkillMatch' + person.Id}
              content={
                <Progress
                  tabIndex={0}
                  className="unselectable"
                  value={getSkillMatch(person.Skills)}
                  color="primary"
                >
                  {getSkillMatch(person.Skills)}%
                </Progress>
              }
            />
          </div>
        ...
        <Row>
          ...
          <Col lg="3" md="6" xs="6">
            <FilterModal
              ...
              setNeededSkill={setNeededSkill}
              toggleSkillMatch={toggleShowSkillMatch}
              notHidden={showSkillMatch}
              hideSkillMatch={hideShowSkillMatch}
              id="filter"
            />
          </Col>
          <Col lg="3" md="6" xs="6">
            <Link style={{ textDecoration: 'none' }} to="/addresource">
              <Button
                ...
                tabIndex={-1}
                className="blue-button btn-block shadow-none"
                id="addResource"
                type="button"
                color="primary"
              >
