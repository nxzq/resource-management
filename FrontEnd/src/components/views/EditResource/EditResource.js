...
class EditResource extends Component {
  ...
  componentDidMount() {
    ...
    axios
      .head(`resources/${params.id}/resume`)
      .then((res) => {
  ...
  handleSubmit = (e) => {
    ...
    axios.put(`resources/${params.id}`, { data }).then((res) => {
  ...
  handleRemoveEducation = (index) => {
    let data = JSON.parse(JSON.stringify(this.state.data))
    if (data.Education.length === 1 || data.Education.length === index)
      data.Education.pop()
    else
      data.Education = [
  ...
  handleRemoveCertification = (index) => {
    let data = JSON.parse(JSON.stringify(this.state.data))
    if (data.Certification.length === 1 || data.Certification.length === index)
      data.Certification.pop()
    else
      data.Certification = [
        ...data.Certification.slice(0, index),
        ...data.Certification.slice(index + 1),
      ]
    this.setState({ data })
  }
  ...
  handleRemoveExperience = (index) => {
    let data = JSON.parse(JSON.stringify(this.state.data))
    if (data.Experience.length === 1 || data.Experience.length === index)
      data.Experience.pop()
    else
      data.Experience = [
  ...
  handleRemoveProject = (index) => {
    let data = JSON.parse(JSON.stringify(this.state.data))
    if (data.Project.length === 1 || data.Project.length === index)
      data.Project.pop()
    else
      data.Project = [
        ...data.Project.slice(0, index),
        ...data.Project.slice(index + 1),
      ]
    this.setState({ data })
  }
          ...
          <div>
            <Container>
              <div>
                ...
                <Link to="/">Return to Resources</Link>
              </div>
            </Container>
          </div>
        ) : (
              ...
              <Container className="ResourceForm">
                <Form onSubmit={this.handleSubmit}>
                  ...
                  <AboutForm
                    ...
                    PersonalSite={this.state.data.PersonalSite}
                  />
                  <SectionHeader name="Resume" />
                  <ResumeUpload
                    resume={this.state.resume}
                    handleResume={this.handleResume}
                    existingResume={this.state.existingResume}
                  ...
                  <SummaryForm
                    handleChange={this.handleChange}
                    SummaryText={this.state.data.SummaryText}
                  />
                  <DynamicSectionHeader
                    name="Education"
                    count={this.state.data.Education.length}
                    addForm={this.handleAddEducation}
                  />
                  {this.state.data.Education.map((element, index) => (
                    <EducationForm
                      ...
                      Major={element.Major}
                      Minor={element.Minor}
                      GradDate={element.GradDate}
                    />
                  ))}
                  <DynamicSectionHeader
                    name="Certifications"
                    count={this.state.data.Certification.length}
                    addForm={this.handleAddCertification}
                  />
                  {this.state.data.Certification.map((element, index) => (
                    <CertificationForm
                      key={index}
                      index={index}
                      removeCertification={this.handleRemoveCertification}
                      handleCertificationChange={this.handleCertificationChange}
                      CertName={element.CertName}
                      CertDate={element.CertDate}
                      CertAssociation={element.CertAssociation}
                    />
                  ))}
                  <DynamicSectionHeader
                    name="Experience"
                    count={this.state.data.Experience.length}
                    addForm={this.handleAddExperience}
                  />
                  {this.state.data.Experience.map((element, index) => (
                    <ExperienceForm
                      ...
                      JobOrg={element.JobOrg}
                      JobStartDate={element.JobStartDate}
                      JobEndDate={element.JobEndDate}
                      JobInfo={element.JobInfo}
                    />
                  ))}
                  <DynamicSectionHeader
                    name="Projects"
                    count={this.state.data.Project.length}
                    addForm={this.handleAddProject}
                  />
                  {this.state.data.Project.map((element, index) => (
                    <ProjectForm
                      ...
                      ProjDate={element.ProjDate}
                      ProjAssociation={element.ProjAssociation}
                      ProjInfo={element.ProjInfo}
                    />
                  ))}
                  ...
                  <Row>
                    ...
                    <Col md="6">
                      <div>
                        ...
                        <Button
                          type="submit"
                          className="blue-button btn-block"
                          color="primary"
                        >
                          Submit Form
                        </Button>
